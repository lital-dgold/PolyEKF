...
from util_func import build_L, vector2diag, compute_metric_summary
...
def safe_single_monte_carlo(cfg, active_methods,  iterable, seed=None):
    """
    Run one repetition of `single_monte_carlo` in total isolation.
    Returns
        (True , result_dict)  on success
        (False, str(msg))     on failure
    """
    try:
        if seed is not None:
            np.random.seed(seed)
        res = single_monte_carlo(cfg, active_methods)
        return True, res
...
def _performance_vs_snr(sigma_w1,base_cfg, n, num_iterations, active_methods):
    ...
    sigma_v = 0.1 * sigma_w1
    C_u = np.dot(sigma_v ** 2, np.eye(cfg["C_u"].shape[0]))
    cfg.update({"C_u": C_u})
...
if __name__ == "__main__":
    ...
    if sparse_flag:
        ...
        C_x_missmatch = np.dot(sigma_x ** 2, np.eye(m))
    mu = 1
    # active_methods = ("gsp-ekf05","gsp-ekf1","gsp-ekf15","gsp-ekf2","gsp-ekf25","gsp-ekf5")#
    active_methods = ("ekf", )#"gsp-ekf", "oracle-block", "change-det")# "change-det")#, "change-det")
    ...
    #
    #     # Save
    #     with open("runs_nonlinear_data_5order_10nodes_1000MC_results_k2n_oracle.pkl", "wb") as f:
    #         pickle.dump(runs_nonlinear_5order, f)
    #     from util_func import plot_metric, create_table
    #
    #     plot_metric(cfg["trajectory_time"], runs_nonlinear_5order, "mse", log_format=True, to_save=True, suffix="10nodes_1000mc_nonlinear_oracle")
    #     # plot_metric(cfg["trajectory_time"], runs_nonlinear, "f1")
    ...
    with simulation("Non-Linear case - Performance vs. noise level"):
        num_iterations = 1000
        poly_coefficients = 0.001 * np.array([0.1, 1.0, 0.8, 0.6, 0.4, 0.2]) #np.array([1.0, 1.0, 1.0, 1.0]) #
        cfg.update({"poly_coefficients": poly_coefficients})
        num_time_samples = 39
        trajectory_time = np.arange(0, num_time_samples)
        cfg.update({"trajectory_time": trajectory_time})
        sigma_w_list = np.logspace(-1.5, 1.5, 4)
        # Run in series
        sparsity_dict_list = []

        for num_edges in sigma_w_list:
            _, result = _performance_vs_snr(num_edges, cfg, n, num_iterations, active_methods)
    ...
    # Save
    with open("performance_vs_snr_5order_10nodes1000MC_.pkl", "wb") as f:
