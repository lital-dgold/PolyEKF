"""
grid_search_oracle_ekf.py
--------------------------
Grid search over sigma_v (C_u), sigma_w (C_w), sigma_x (C_x), and new_edge_weight
for the oracle-block EKF on IEEE57 data.
...
Outputs (saved to SAVE_DIR):
  traj_*.png          — trajectory plots for the top N_PLOT_BEST combos by avg_mse
  heatmap_<p1>_<p2>_<metric>.png  — 2D heat-maps for every parameter pair (min over the other 2)
  grid_results.pkl    — raw results dict keyed by (sigma_v, sigma_w, sigma_x, new_edge_weight)
  summary.txt         — ranked table of all combos
"""
...
SIGMA_V_GRID = [0.01, 0.1, 1.0, 5.0]    # process noise  → C_u = σ_v² I_m
SIGMA_W_GRID = [0.01, 0.1, 1.0, 5.0]    # measurement noise → C_w = σ_w² I_n
SIGMA_X_GRID = [1.0, 5.0, 10.0]         # initial state uncertainty → C_x = σ_x² diag(s0)

NEW_EDGE_WEIGHT = None  # None = use mean of actual edge weights from data (matches power_system_tracking.py)

T_MAX         = 100    # only run the first T_MAX time steps
WINDOW_A      = (0, 100)
N_PLOT_BEST   = 10     # save trajectory plots only for this many top combos

DATASET_PATTERN = "Power_data/ieee57_dataset*"
N_DATASETS_MAX  = 2    # None = all datasets

SAVE_DIR = "Power_data/grid_search_oracle_ekf"
...
def _run_dataset(ds_folder, sigma_v, sigma_w, sigma_x):
    """Run oracle-block EKF on one dataset; return MSE trajectory (T,)."""
    ...
    ew = _mean_edge_weight if NEW_EDGE_WEIGHT is None else NEW_EDGE_WEIGHT
    stateInit_mc           = ew * conn_mc[0, :].reshape(-1, 1)

    cfg_run = copy.deepcopy(_cfg_base)
    cfg_run["stateInit"]       = stateInit_mc
    cfg_run["new_edge_weight"] = ew
    cfg_run["C_u"] = (sigma_v ** 2) * np.eye(cfg_run["m"])
    cfg_run["C_w"] = (sigma_w ** 2) * np.eye(cfg_run["n"])
    cfg_run["C_x"] = (sigma_x ** 2) * vector2diag(stateInit_mc)

    filt = METHOD_REGISTRY["oracle-block"](cfg_run)
...
def _run_dataset_task(args):
    ...
    ds_folder, sigma_v, sigma_w, sigma_x = args
    traj = _run_dataset(ds_folder, sigma_v, sigma_w, sigma_x)
    return sigma_v, sigma_w, sigma_x, traj
...
def _plot_trajectory(trajectories, params, avg_mse, gap, save_path):
    sigma_v, sigma_w, sigma_x = params
    ...
    ax.set_ylabel("MSE")
    ax.set_title(
        f"Oracle EKF  σ_v={sigma_v}  σ_w={sigma_w}  σ_x={sigma_x}\n"
...
def _save_heatmap_2d(results_grid, metric_key, param_i, param_j,
                     grid_i, grid_j, param_names, all_grids):
    """
    2D heatmap for parameters param_i vs param_j.
    Projects over the remaining parameter(s) by taking the minimum value.
    """
    ...
    for ri, vi in enumerate(grid_i):
        for rj, vj in enumerate(grid_j):
            # collect all keys matching these two params, minimize over the others
            candidates = []
    ...
    ax.set_title(f"{metric_label} — Oracle EKF  (min over {other_names})")
    ...
    plt.tight_layout()
    fname = f"heatmap_{metric_key}_{ylabel}_vs_{xlabel}.png"
...
if __name__ == "__main__":
    ...
    param_grid = list(itertools.product(SIGMA_V_GRID, SIGMA_W_GRID, SIGMA_X_GRID))
    ...
    logging.info(f"Datasets: {len(dataset_dirs)}  |  "
                 f"Combos: {n_combos}  |  Tasks: {n_tasks}  |  Workers: {n_workers}")

    tasks = [
        (ds, sv, sw, sx)
        for sv, sw, sx in param_grid
        for ds in dataset_dirs
    ]

    # ── Parallel execution ─────────────────────────────────────────────────────
    raw: dict = {}   # (sv, sw, sx) -> list of trajectories
    t_wall = time.time()
    done   = 0
    ...
    with ProcessPoolExecutor(max_workers=n_workers) as pool:
        ...
        for fut in as_completed(futures):
            ...
            ds, sv, sw, sx = futures[fut]
            try:
                sv_, sw_, sx_, traj = fut.result()
                raw.setdefault((sv_, sw_, sx_), []).append(traj)
            except Exception as exc:
                logging.warning(f"  [{done}/{n_tasks}] Skipped {ds} "
                                f"σ_v={sv} σ_w={sw} σ_x={sx}: {exc}")
                continue
            logging.info(f"  [{done}/{n_tasks}]  σ_v={sv}  σ_w={sw}  σ_x={sx}"
    ...
    # results_grid = pickle.load(open(pkl_path, "rb"))


    # ── Trajectory plots — top N_PLOT_BEST combos ──────────────────────────────
    ranked = sorted(results_grid.items(), key=lambda kv: kv[1]["avg_mse"])
    for rank, (params, res) in enumerate(ranked[:N_PLOT_BEST]):
        sv, sw, sx = params
        fname = f"traj_rank{rank+1:02d}_sv{sv}_sw{sw}_sx{sx}.png"
        _plot_trajectory(res["trajectories"], params,
                         res["avg_mse"], res["gap_a"],
                         os.path.join(SAVE_DIR, fname))

    # ── 2D heatmaps for every parameter pair ──────────────────────────────────
    param_names = ["sigma_v", "sigma_w", "sigma_x"]
    all_grids   = [SIGMA_V_GRID, SIGMA_W_GRID, SIGMA_X_GRID]
    ...
    for (pi, pj) in itertools.combinations(range(3), 2):
        for metric_key in ("avg_mse", "gap_a"):
            _save_heatmap_2d(results_grid, metric_key, pi, pj,
                             all_grids[pi], all_grids[pj],
                             param_names, all_grids)

    # ── Summary table ─────────────────────────────────────────────────────────
    summary_path = os.path.join(SAVE_DIR, "summary.txt")
    with open(summary_path, "w") as fh:
        header = f"{'rank':>4}  {'sigma_v':>8}  {'sigma_w':>8}  {'sigma_x':>8}  {'avg_mse':>10}  {'gap':>10}\n"
        fh.write(header)
        fh.write("-" * len(header) + "\n")
        for rank, (params, res) in enumerate(ranked):
            sv, sw, sx = params
            fh.write(f"{rank+1:>4}  {sv:>8}  {sw:>8}  {sx:>8}  "
                     f"{res['avg_mse']:>10.4f}  {res['gap_a']:>10.4f}\n")
    logging.info(f"Saved summary → {summary_path}")

    # ── Print top 5 ───────────────────────────────────────────────────────────
    print("\n══════════════ Oracle EKF Grid Search — Top 5 by avg MSE ══════════════")
    for rank, (params, res) in enumerate(ranked[:5]):
        sv, sw, sx = params
        print(f"  #{rank+1}  σ_v={sv:<5}  σ_w={sw:<5}  σ_x={sx:<5}"
