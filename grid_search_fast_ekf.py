...
Outputs (saved to SAVE_DIR):
  ...
  grid_results.pkl                    — raw results dict keyed by (sv, sw, sx)
...
METHOD_TO_OPT = "fast-ekf"
# ─── Grid search configuration ────────────────────────────────────────────────
SIGMA_V_GRID    = [0.01, 0.1, 1.0, 5]     # process noise     → C_u = σ_v² I_m
SIGMA_W_GRID    = [0.01, 0.1, 1.0, 5]     # measurement noise → C_w = σ_w² I_n
...
def _plot_trajectory(trajectories, params, avg_mse, gap, save_path):
    ...
    ax.set_ylabel("MSE")
    ax.set_title(
        f"GSP-EKF  σ_v={sigma_v}  σ_w={sigma_w}  σ_x={sigma_x} \n"
...
if __name__ == "__main__":
    ...
    for rank, (params, res) in enumerate(ranked[:5]):
        sv, sw, sx,  = params
        print(f"  #{rank+1}  σ_v={sv:<6}  σ_w={sw:<6}  σ_x={sx:<5}"
